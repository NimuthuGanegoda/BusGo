...
import { Download, RefreshCw, Shield, FileText } from 'lucide-react';
...
const securityStyles: Record<string, { bg: string; color: string; label: string }> = {
  ...
  // UFR_51
  REPEATED_QR_SCAN:   { bg: '#fef3c7', color: '#d97706', label: '🔁 REPEATED QR SCAN' },
  CAPACITY_VIOLATION: { bg: '#fef2f2', color: '#dc2626', label: '🚨 CAPACITY VIOLATION' },
};
...
export default function AuditLogs() {
  ...
  const fetchSecLogs = useCallback(async () => {
    setSecLoading(true);
    try {
      const params = new URLSearchParams({ page: String(secPage), page_size: String(PAGE_SIZE) });
      if (secFilter !== 'all') params.set('event_type', secFilter);
      if (sevFilter !== 'all') params.set('severity', sevFilter);
      const res  = await fetch(`${API}/security-logs?${params}`, { headers: { Authorization: `Bearer ${token()}` } });
      const json = await res.json();
    ...
    <div className="audit-page">
      <div className="audit-header">
        ...
        <div className="audit-filters">
          {/* Tab switcher */}
          <div style={{ display: 'flex', background: '#f3f4f6', borderRadius: '10px', padding: '3px', marginRight: '12px' }}>
          ...
          {activeTab === 0 ? (
            ...
            <>
              <select value={secFilter} onChange={e => { setSecFilter(e.target.value); setSecPage(1); }} className="audit-filter">
                ...
                <option value="REPEATED_QR_SCAN">Repeated QR Scan</option>
              ...
              <table className="audit-table">
                <thead>
                  <tr>
                    ...
                    <th>TABLE</th><th>RECORD ID</th><th>DETAILS</th>
                  </tr>
                </thead>
                <tbody>
                  {logs.length === 0 && (
                  ...
                  {logs.map(log => {
                    ...
                    return (
                      <tr key={log.id}>
                        ...
                        <td><span className="audit-action-badge" style={{ background: style.bg, color: style.color }}>{style.label}</span></td>
          ...
          <div className="audit-table-wrap">
            {secLoading ? (
              ...
              <table className="audit-table">
                <thead>
                  <tr>
                    <th>TIMESTAMP</th><th>EVENT</th><th>EMAIL</th>
                    <th>IP ADDRESS</th><th>SEVERITY</th><th>DETAILS</th>
                  </tr>
                </thead>
                <tbody>
                  {secLogs.length === 0 && (
                  ...
                  {secLogs.map(log => {
                    ...
                    return (
                      <tr key={log.id}>
                        <td className="audit-timestamp">{new Date(log.created_at).toLocaleString('en-LK', { timeZone: 'Asia/Colombo' })}</td>
                        <td><span className="audit-action-badge" style={{ background: style.bg, color: style.color }}>{style.label}</span></td>
                        <td className="audit-admin"><div>{log.email || '—'}</div></td>
                        <td style={{ fontFamily: 'monospace', fontSize: '12px', color: '#6b7280' }}>{log.ip_address || '—'}</td>
                        <td>
                          <span style={{
