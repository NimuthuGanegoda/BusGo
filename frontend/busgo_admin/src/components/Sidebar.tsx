...
const DEV_NAV = [
  { to: '/admin/recovery-requests', icon: KeyRound, label: 'Recovery Requests' },
];
...
export default function Sidebar({ open, onClose }: Props) {
  ...
  return (
    <aside style={{
      ...
      padding: '1.5rem 1rem', borderRight: '1px solid var(--border)',
      zIndex: 100, transition: 'transform 0.3s ease',
      transform: open ? 'translateX(0)' : undefined, overflowY: 'auto',
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.85rem' }}>
          ...
          <div>
            <div style={{ fontSize: '1.2rem', fontWeight: 700, color: 'var(--text)', lineHeight: 1 }}>BUSGO</div>
            <div style={{ fontSize: '0.65rem', color: 'var(--text-muted)', letterSpacing: '0.1em', textTransform: 'uppercase' }}>
              {isDeveloper ? 'Developer Panel' : 'Admin Panel'}
            </div>
          </div>
        </div>
          ...
          {NAV.map(({ to, icon: Icon, label }) => (
            <li key={to}>
              <NavLink to={to} onClick={onClose} style={({ isActive }) => navLinkStyle(isActive)}>
                {({ isActive }) => (<><Icon size={18} strokeWidth={isActive ? 2.5 : 1.8} /><span>{label}</span></>)}
              </NavLink>
            </li>
          ))}
        </ul>
        ...
        {isDeveloper && (
          <>
            <div style={{
              ...
              letterSpacing: '0.12em', textTransform: 'uppercase',
              padding: '0 0.5rem', margin: '1.2rem 0 0.5rem',
            }}>Developer Tools</div>
            <ul style={{ listStyle: 'none', display: 'flex', flexDirection: 'column', gap: '4px' }}>
              {DEV_NAV.map(({ to, icon: Icon, label }) => (
                <li key={to}>
                  <NavLink to={to} onClick={onClose} style={({ isActive }) => ({
                    ...
                    background: isActive ? '#d97706' : 'rgba(245,158,11,0.08)',
                    color: isActive ? '#fff' : '#f59e0b',
                    border: `1px solid ${isActive ? '#d97706' : 'rgba(245,158,11,0.2)'}`,
                  })}>
                    {({ isActive }) => (<><Icon size={18} strokeWidth={isActive ? 2.5 : 1.8} /><span>{label}</span></>)}
                  </NavLink>
                </li>
      ...
      <div style={{ borderTop: '1px solid var(--border)', paddingTop: '1rem' }}>
        <div style={{
          ...
          background: 'var(--surface-raised)', padding: '0.75rem',
          borderRadius: 'var(--radius-sm)', marginBottom: '0.75rem',
        }}>
          <div style={{
            width: 36, height: 36, borderRadius: '50%',
            background: isDeveloper ? '#d97706' : 'var(--primary)',
        ...
        <button onClick={handleLogout} style={{
          ...
          onMouseEnter={e => { e.currentTarget.style.background = 'rgba(220,53,69,0.12)'; e.currentTarget.style.color = 'var(--danger)'; }}
      ...
      <style>{`
        @media (max-width: 992px) {
          .busgo-sidebar { transform: ${open ? 'translateX(0)' : 'translateX(-100%)'} !important; }
