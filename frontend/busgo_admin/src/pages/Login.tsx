...
function NeonToast({ toast, onClose }: { toast: ToastData; onClose: () => void }) {
  ...
  return (
    <div onClick={onClose} style={{
      ...
      boxShadow: `0 0 12px ${cfg.glow}40`, borderRadius: '10px',
      padding: '14px 16px', marginBottom: '10px',
      display: 'flex', alignItems: 'flex-start', gap: '12px',
      animation: 'slideDown 0.4s ease-out', cursor: 'pointer',
    }}>
      <div style={{ width: '3px', height: '40px', background: cfg.text, borderRadius: '2px', flexShrink: 0 }} />
      <div style={{ fontSize: '18px', color: cfg.text, flexShrink: 0, marginTop: '2px' }}>{cfg.icon}</div>
      <div style={{ flex: 1 }}>
        <div style={{ color: cfg.text, fontWeight: 700, fontSize: '14px' }}>{toast.title}</div>
        <div style={{ color: cfg.text, opacity: 0.8, fontSize: '12px', marginTop: '2px' }}>{toast.message}</div>
      </div>
    </div>
  );
}
...
export default function Login() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    for (let attempt = 1; attempt <= 2; attempt++) {
      try {
        ...
        if (res.status === 429) { setLoading(false); addToast('warning', 'Slow Down', 'Too many attempts.'); return; }
        if (!res.ok) { setLoading(false); addToast('error', 'Login Failed', 'Invalid email or password.'); return; }
        if (!['admin', 'developer'].includes(data.data?.user?.role)) {
          setLoading(false); addToast('error', 'Access Denied', 'Not authorized to access this panel.'); return;
        }
        ...
        addToast('success', 'Welcome!', 'Logged in. Redirecting...');
        setTimeout(() => { window.location.href = '/admin/dashboard'; }, 1000);
        return;
      } catch {
        if (attempt < 2) { await new Promise(r => setTimeout(r, 800)); continue; }
        setLoading(false); addToast('warning', 'Connection Error', 'Could not reach the server.'); return;
      }
    }
    setLoading(false);
  };
  ...
  useEffect(() => {
    ...
    if (!el) return;
    const setDashoffset = (p: SVGGeometryElement) => {
      const l = p.getTotalLength(); p.setAttribute('stroke-dasharray', String(l)); return [l, 0];
    };
    el.classList.add('ready');
    const letters = anime({
      targets: '#splash-lines path',
      strokeDashoffset: { value: (p: any) => setDashoffset(p), duration: 700, easing: 'easeOutQuad' },
      transform: ['translate(0 128)', 'translate(0 0)'],
      delay: (_: any, i: number) => 600 + i * 140, duration: 1400,
    });
    const fills = anime({
      targets: '#splash-fills *', opacity: [0, 1],
      delay: (_: any, i: number, l: number) => ((letters as any).duration - 200) + Math.abs(i - l / 2) * 40,
      duration: 500, easing: 'linear',
    });
    anime({ targets: '#splash-dot', transform: ['translate(0 -300) scale(1 3)', 'translate(0 0) scale(1 1)'],
      opacity: [0, 1], duration: 600, easing: 'easeOutBounce', delay: (letters as any).duration - 200 });
    anime({ targets: '.splash-subtitle', opacity: [0, 1], translateY: [20, 0],
      duration: 500, easing: 'easeOutQuad', delay: (fills as any).duration + 200 });
    setTimeout(() => { setSplashFading(true); setTimeout(() => setSplashVisible(false), 1000); },
      (fills as any).duration + 1200);
  }, []);
  ...
  useEffect(() => {
    ...
    ids.forEach((id, i) => {
      const line = bus.querySelector(id) as SVGLineElement; if (!line) return;
  ...
  return (
    <div className="login-page">
      <div style={{ position: 'fixed', top: '20px', right: '20px', zIndex: 9999, width: '380px', maxWidth: '90vw' }}>
        {toasts.map(t => <NeonToast key={t.id} toast={t} onClose={() => removeToast(t.id)} />)}
      </div>
            ...
            <svg className="splash-logo" width="36rem" height="12rem" viewBox="0 0 640 200">
              <defs>
                <radialGradient cx="50%" cy="0%" fx="50%" fy="0%" r="50%" id="rg-indigo">
                  <stop stopColor="#6a70e0" offset="0%"/><stop stopColor="#3b3f8f" offset="100%"/>
                </radialGradient>
                <radialGradient cx="50%" cy="0%" fx="50%" fy="0%" r="50%" id="rg-teal">
                  <stop stopColor="#40d8b0" offset="0%"/><stop stopColor="#1a8a7a" offset="100%"/>
                </radialGradient>
                <radialGradient cx="50%" cy="0%" fx="50%" fy="0%" r="100%" id="rg-green">
                  <stop stopColor="#6ae894" offset="0%"/><stop stopColor="#3da55c" offset="100%"/>
                </radialGradient>
              </defs>
              <g stroke="none" strokeWidth="1" fill="none" fillRule="evenodd">
                ...
                <g id="splash-lines" transform="translate(24, 36)">
                  <path d="M 16,128 L 16,0" stroke="#3b3f8f" strokeWidth="32" strokeLinecap="round"/>
                  <path d="M 16,0 C 58,0 58,64 16,64" stroke="#6a70e0" strokeWidth="32"/>
                  <path d="M 16,64 C 66,64 66,128 16,128" stroke="#3b3f8f" strokeWidth="32"/>
                  <path d="M 112,0 L 112,80 C 112,106.5 133.5,128 160,128 C 186.5,128 208,106.5 208,80 L 208,0" stroke="#1a8a7a" strokeWidth="32"/>
                  <path d="M 304,28 C 304,8 288,0 272,0 C 256,0 240,10 240,32 C 240,56 260,66 280,72 C 296,78 312,88 312,104 C 312,126 296,128 276,128 C 260,128 244,118 240,100" stroke="#3da55c" strokeWidth="32"/>
                  <path d="M 408,36 C 398,12 384,0 368,0 C 345,0 336,28.6 336,64 C 336,99.3 345,128 368,128 C 384,128 398,116 408,92 L 408,64 L 372,64" stroke="#3b3f8f" strokeWidth="32"/>
                  <path d="M 496,0 C 522.5,0 544,28.6 544,64 C 544,99.3 522.5,128 496,128 C 469.5,128 448,99.3 448,64 C 448,28.6 469.5,0 496,0 Z" stroke="#1a8a7a" strokeWidth="32"/>
                </g>
      ...
      <div className="login-right">
        <div className="login-right-content">
          <div className="login-hero-logo-wrap"><img src={busgoLogo} alt="" className="login-hero-logo" /></div>
