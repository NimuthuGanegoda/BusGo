...
export async function createAlert(userId, dto) {
  ...
  // ── Notification (sent immediately — before ML completes) ─────────────────
  ...
  // ── ML prioritization — runs in background, does NOT block the response ────
  // SBERT embedding takes 5-13 seconds. Running it via setImmediate prevents
  // the Dio receiveTimeout on the Flutter client. The admin panel receives the
  // alert immediately and the ml_priority field is updated within ~10 seconds.
  setImmediate(async () => {
    try {
      ...
      console.log('[Emergency ML] Background classification complete:', JSON.stringify(mlResult));
      ...
      // ML failure does not affect the alert — it remains saved and visible
      console.error('[Emergency ML] Background classification failed:', mlErr.message);
    }
  });

  // ── Return immediately — response sent before ML completes ────────────────
